...
def read_joint_transforms_xml(filename):
    ...
    for joint_elem in root.findall('joint'):
        ...
        for key_elem in joint_elem.findall('key'):
            ...
            wquat = converter * wp.quat(quaternion)
...
class Example:
    ...
    END_EFFECTOR_NAMES = ("left_shoulder", 
                          "left_elbow", 
                          "left_wrist", 
                          "right_shoulder", 
                          "right_elbow", 
    ...
    def __init__(
        ...
        self.joint_data = read_joint_transforms_xml('D:/src/newton/animation/Unitree_JumpsForward.xml')
        #print(self.joint_data)
        
        self.frame_count = len(self.joint_data['Ctrl_Hips'])
    ...
    def _build_model(num_envs: int):
        ...
        # torso, waist
        ee_link_indices = [18, 19, 22, 25, 26, 29, 3, 4, 6, 9, 10, 12, 15, 14]  
        ee_link_offsets = [wp.vec3()] * 14
    ...
    def _initialize_targets(self):
        ...
        for env in range(self.num_envs):
            ...
            for ee_idx, link_idx in enumerate(self.ee_link_indices):
                ...
                print (f"link_idx: {link_idx} tf: {tf}")
                ...
                print(f"environment {env} effeector {ee_idx} init target rotation: {tgt_rot[env, ee_idx]} and init target position: {tgt_pos[env, ee_idx]}, link_offset: {self.ee_link_offsets[ee_idx]}")
    ...
    def _advance_animation(self):
        ...
        for ee_idx, joint_name in enumerate(MATCHING_JOINT_NAMES):
            keyframes = self.joint_data[joint_name]
            if len(keyframes) > 0:
                keyframe = keyframes[current_frame // 3]
                base_offset = offset + self.joint_data["Ctrl_Hips"][0]['position']
                base_offset[2] = 0.0
                position = base_offset + keyframe['position']
                rotation = keyframe['quaternion']
                #print (f"joint name: {joint_name} position: {position}, rotation: {rotation}")
                self.target_positions[0, ee_idx] = position
                #self.target_rotations[0, ee_idx] = rotation
                self.position_objectives[ee_idx].set_target_position(0, wp.vec3(*position))

                # update gizmo
                gid = env_id * len(self.END_EFFECTOR_NAMES) + ee_idx
                self.gizmo_system.update_target_position(gid, position)
                #self.gizmo_system.update_target_rotation(gid, rotation)
                #self.rotation_objectives[ee_idx].set_target_rotation(0, wp.vec4(*rotation))
